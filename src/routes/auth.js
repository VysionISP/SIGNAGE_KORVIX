...
route('GET', '/api/auth/state', (req, res) => {
  ...
  sendJson(res, 200, { needs_setup: !hasUsers });
});
...
route('POST', '/api/auth/setup', async (req, res) => {
  ...
  db.logEvent('auth.setup', { detail: body.email });
...
route('POST', '/api/auth/login', async (req, res) => {
  ...
  db.logEvent('auth.login', { detail: user.email });
...
route('POST', '/api/orgs', async (req, res) => {
  ...
  db.logEvent('org.created', { detail: body.name });
...
route('PATCH', '/api/orgs/:id', async (req, res, params) => {
  ...
  const body = await readJson(req);
  db.run('UPDATE orgs SET name = ? WHERE id = ?', body.name ?? org.name, org.id);
...
route('POST', '/api/users', async (req, res) => {
  ...
  db.logEvent('user.created', { detail: `${body.email} (${role})` });
...
route('DELETE', '/api/users/:id', (req, res, params) => {
  ...
  db.logEvent('user.deleted', { detail: target.email });
  sendJson(res, 200, { ok: true });
});
